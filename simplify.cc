...
namespace client {
  ...
  collapsed_type collapse_visitor::operator()(binary_operation & t) const {
    collapsed_type c = boost::apply_visitor(collapse_visitor(), t.left.type);
    if (c.can_collapse)
      t.left.type = c.value;

    c = boost::apply_visitor(collapse_visitor(), t.right.type);
    if (c.can_collapse)
  ...
  collapsed_type collapse_visitor::operator()(unary_operation & t) const {
    collapsed_type c = boost::apply_visitor(collapse_visitor(), t.tree.type);
    if (c.can_collapse)
      t.tree.type = c.value;

    num_type tree = boost::apply_visitor(num_visitor(), t.tree.type);
    if (tree)
